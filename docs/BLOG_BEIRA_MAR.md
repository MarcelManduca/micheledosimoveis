...
Para evitar abrir indiscriminadamente a tabela `properties` pública com `published = false`:
...
3. **Resolução Persistente Autorizada (`resolveEditorialPreservedSnapshot`):**
   - O banco de dados é a ÚNICA fonte de verdade pública.
   - Se o registro estiver com `is_preserved = false` ou `is_admin_blocked = true`, o PostgreSQL via RLS retorna `data = null`.
   - O resolvedor identifica que o banco retornou vazio e retorna `null` (HTTP 404).
   - **Zero Fallback Público:** Em caso de tabela ausente, erro de schema cache ou ausência de cliente, a resolução retorna `null`, sem expor dados estáticos não confirmados.
   - A semente estática (`EDITORIAL_PRESERVED_CATALOG`) serve exclusivamente para a carga inicial idempotente (`20260913000001_seed_editorial_preserved_properties.sql`).
4. **Diferenciação de Estados:**
   - `is_preserved = false`: Desativa a exibição do acervo editorial (retorna 404 para snapshot), sem afetar a listagem comercial se ativa.
   - `is_admin_blocked = true` (Bloqueio Administrativo Total): Prevalece sobre `properties.published = true`, bloqueando a unidade em todas as pontas (detalhe 404, buscas, MCP, recomendações, sitemap e sincronizador).
...
1. **No Banco de Dados / Runtime:**
   ...
   - Isso bloqueia imediatamente `getPropertyByCode` (404), ferramentas MCP, buscas, recomendações e `sitemap.xml`.
