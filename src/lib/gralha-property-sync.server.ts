...
export async function syncOneGralhaProperty(
  ...
  // Resolver primeiro o codigo da propriedade
  const finalUrlCode = urlCode || "";

  // Buscar registro atual por codigo ou URL
  ...
  // 3. Tratar imóveis não encontrados / indisponíveis
  if (isNotFound) {
    if (existing) {
      await db
        .from("properties")
  ...
  // 4. Imóvel disponível -> Realizar parse do HTML e sincronizar campos
  let scraped;
  try {
    scraped = await parseGralhaPropertyHtml(html!, url);
  } catch (err) {
    if (existing) {
      await db
        .from("properties")
  ...
  if (existing) {
    ...
    // Se mudou de publicado=false para true, ou mudou status
    ...
    // Atualizar last_checked_at e last_check_status
    ...
    if (urlsToDelete.length > 0) {
      await db.from("property_photos").delete().eq("property_id", existing.id).in("url", urlsToDelete);
    }
    ...
    for (let i = 0; i < desiredPhotos.length; i++) {
      ...
      if (!exist) {
        ...
        if (exist.position !== i) {
          await db.from("property_photos").update({ position: i }).eq("id", exist.id);
          photosReordered++;
        }
      }
    }
    ...
    if (toInsert.length > 0) {
      await db.from("property_photos").insert(toInsert);
    }
    ...
    if (desiredPhotos.length > 0) {
      ...
      await db.from("property_photos").insert(rows);
    }
